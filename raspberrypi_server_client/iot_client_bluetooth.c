...
int main(int argc, char *argv[])
{
	...
	char dest[18] = "98:DA:60:0D:91:91";	//iot25
	char msg[BUF_SIZE];
...
//			sprintf(name_msg,"[%s]%s",dev_fd->sendid,msg);
  			//fputs("ARD:",stdout);
			fputs(msg,stdout);
			if(write(dev_fd->sockfd, msg, strlen(msg))<=0) //server close
			{
				dev_fd->sockfd = -1;
				return NULL;
			}
...
void * recv_msg(void * arg)  //server --> bluetooth
{
	...
	//pthread_create(&led_thread, NULL, led_msg, (void *)&dev_fd);
	//void * thread_return;
	while(1) {
...
/*
void * led_msg(void * arg) //check used_seat form sql
{
	MYSQL* conn;
	MYSQL_ROW sqlrow;
	int res;
